...
export const emailPattern = defineRegexPattern(
  String.raw`${EMAIL_LOCAL_PART}@${EMAIL_DOMAIN}`,
);

export function isEmail(value: string, flags?: string): boolean {
  return emailPattern.test(value, flags);
}

export class EmailRegex extends RegExp {
  constructor(flags?: string) {
    super(emailPattern.exactSource, flags);
  }
}
